...
2\. Choose any "eligible" node k, and assign it a label pk as follows:

- pk = min {dkj + pj}, the minimum taken over all successors j of node k

3\. Repeat Step 2 until the source node is labeled. The label on the source _is_ the shortest distance from the source to the destination.

Insert Figure 3.15 here

|                                                   |
| ------------------------------------------------- |
| **Figure 3.15** Acyclic network with node labels. |
...
Observe that the label on each node summarizes information on higher-numbered nodes. In fact, the value of the label on any node _is_ actually the length of the shortest path from that node to the destination.
